...
from ..MANTENIMIENTO_PREVENTIVO_CORRECTIVO_SOLICITUDES import (
  MANTENIMIENTO_PREVENTIVO_CORRECTIVO_SOLICITUDES,
)
from ..MANTENIMIENTO_PREVENTIVO_CORRECTIVO_REPORTE import (
  MANTENIMIENTO_PREVENTIVO_CORRECTIVO_REPORTE,
)
from ..MANTENIMIENTO_PREVENTIVO_CORRECTIVO_SOLICITUDES import (
  MANTENIMIENTO_PREVENTIVO_CORRECTIVO_SOLICITUDES,
)
...
class MANTENIMIENTO_PREVENTIVO_CORRECTIVO_SOLICITUDES_REGISTROS_copy(
  MANTENIMIENTO_PREVENTIVO_CORRECTIVO_SOLICITUDES_REGISTROS_copyTemplate
):
  ...
  def abrir_reporte(self, datos, **event_args):
    ...
    respuesta = alert(
      content=MANTENIMIENTO_PREVENTIVO_CORRECTIVO_REPORTE(datos),
      large=True,
      dismissible=False,
      buttons=[("SALIR", True)],
      role="wide-modal-content-bigger",
    )
  ...
  def abrir_solicitud(self, datos, **event_args):
    datos.update(self.datos)
    respuesta = alert(
      content=MANTENIMIENTO_PREVENTIVO_CORRECTIVO_SOLICITUDES(datos),
      large=True,
      dismissible=False,
      buttons=[("SALIR", True)],
      role="wide-modal-content-bigger",
  ...
  def programar_mantenimiento(self, datos, **event_args):
    with Notification(
      "Registrando fecha en la base de datos...", title="GUARDANDO.", style="info"
    ):
      registro_actual = None
      for item in self.registros_mtto:
        if (
          item["id_solicitud_mtto"] == datos["id_solicitud_mtto"]
          and item["registro_principal"] == "1"
        ):
    ...
    Notification(
      "Fecha registrada correctamente!", title="ÉXITO!.", style="success"
    ).show()
    self.button_actualizar_click()
  ...
  def filtros(self):
    ...
    if len(self.text_box_filtro_folio.text) > 0:
      items = [
        item
        for item in items
        if str(self.text_box_filtro_folio.text).upper() in str(item["folio"])
      ]
    if self.date_picker_filtro_fecha_programada.date != None:
      items = [
        item
        for item in items
        if (self.date_picker_filtro_fecha_programada.date).strftime("%d/%m/%Y")
        in str(item["fecha_programada"])
      ]
    if len(self.text_box_filtro_area.text) > 0:
      items = [
        item
        for item in items
        if str(self.text_box_filtro_area.text).upper() in str(item["area"])
      ]
    if len(self.text_box_filtro_equipo.text) > 0:
      items = [
        item
        for item in items
        if str(self.text_box_filtro_equipo.text).upper() in str(item["equipo"])
      ]
    if self.drop_down_status.selected_value != None:
      items = [
        item
        for item in items
        if (int(item["mtto_realizado"]) + int(item["vobo_solicitante"]))
    ...
    if len(self.text_box_filtro_descripcion_anomalia.text) > 0:
      items = [
        item
        for item in items
        if str(self.text_box_filtro_descripcion_anomalia.text).upper()
        in str(item["descripcion_anomalia"]).upper()
      ]
    if len(self.text_box_filtro_persona_reporta.text) > 0:
      items = [
        item
        for item in items
        if str(self.text_box_filtro_persona_reporta.text).upper()
        in str(item["persona_reporta"]).upper()
      ]
  ...
  def leer_registros(self):
    ...
    if self.datos["id_usuario_erp"] != 58 and self.datos["id_usuario_erp"] != 884:
      for row in self.ws_consulta_solicitudes_mtto.rows:
        if (
          row["persona_reporta"] == self.datos["nombre_usuario"]
          or row["nombre_testigo"] == self.datos["nombre_usuario"]
        ):
          self.registros_consulta_mtto.append(row)
    else:
      ...
      if self.datos["area"] == "todas":
        ...
        if self.datos["equipo"] == "todos":
          ...
          for row in self.ws_consulta_solicitudes_mtto.rows:
            if (
              row["equipo"] == self.datos["equipo"]
              and row["area"] == self.datos["area"]
            ):
              self.registros_consulta_mtto.append(row)
    ...
    if len(self.registros_consulta_mtto) > 0:
      for fila in self.registros_consulta_mtto:
        ...
        dic_fila["id_usuario_erp"] = self.datos["id_usuario_erp"]
        dict_registros_consulta_mtto.append(dic_fila)
      self.repeating_panel_registros.items = (
        dict_registros_consulta_mtto  # self.registros_consulta_mtto
      )
      self.column_panel_empty_db.visible = False
      self.data_grid_registros.visible = True
