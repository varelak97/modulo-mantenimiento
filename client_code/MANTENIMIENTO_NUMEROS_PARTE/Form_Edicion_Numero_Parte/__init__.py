...
class Form_Edicion_Numero_Parte(Form_Edicion_Numero_ParteTemplate):
  ...
  def button_guardar_click(self, **event_args):
    modos = [{'tag':'id_cliente', 'index':0}]
    status = Funciones_Globales.validar_campos( self.lista_componentes, self.registro_actual, self.campos_no_obligatorios, self.datos['modo'], modos, 'id_herramental')
    if status == 1:
